...
async def start(update:Update,context:ContextTypes.DEFAULT_TYPE):
	await update.message.reply_text("Hello! Welcome to my weather forcast bot.")

async def weather(update:Update,context:ContextTypes.DEFAULT_TYPE):
	city=" ".join(context.args)
	if not city:
		await update.message.reply_text("City not provided. please provide a city.")
		logger.error("City not provided. please provide a city.")
		return None
	...
	await update.message.reply_text(f"Weather fetched successfully. \ncity: {city_name}\ncountry: {country}\ntemprature in Celcius: {temp_c}\ncondition: {condition} ")